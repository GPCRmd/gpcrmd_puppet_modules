class solr {
    ...
    # download solr
        ...
        exec { "install-solr":
            cwd=> "/protwis/conf/protwis_puppet_modules/solr/",
            command => "bash ./install_solr_service.sh solr*.tgz",
    ...
    exec { "build-indexes":
        ...
        require => [Exec["import-db-dump"],Exec["restart-solr"],Python::Puppet::Install::Pip[$python::pip_packages],Exec["install-rdkit"]],
