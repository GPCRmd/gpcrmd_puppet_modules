...
class import_db {
    ...
    exec { "create-postgres-user":
        ...
        require => $osfamily ? {
            "Debian" => [ Package["postgresql-${Postgresql::postgresql_version}", "expect"], Service[$Postgresql::postgresql_service_name] ],
            "RedHat" => [ Package["postgresql${Postgresql::postgresql_version2}", "expect"], Exec["start-postgres-server"], Service[$Postgresql::postgresql_service_name] ],
        }
    }
